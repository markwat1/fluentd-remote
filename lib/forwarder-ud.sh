...
echo '<match td.messages.syslog.*>' > /etc/td-agent/td-agent.conf
...
echo '  port __SYSLOG_PORT__' >> /etc/td-agent/td-agent.conf
echo '  bind 0.0.0.0' >> /etc/td-agent/td-agent.conf
echo '  @id input_forward' >> /etc/td-agent/td-agent.conf
echo '  tag td.messages.syslog' >> /etc/td-agent/td-agent.conf
echo '</source>' >> /etc/td-agent/td-agent.conf
